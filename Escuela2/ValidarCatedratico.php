...
!empty($_POST['opSexo'])and!empty($_POST['txtCarrera'])
    ...
    $con=Conecta();
	$C1="SELECT * FROM catedratico where matricula='$txtMatricula'";
	$R1=pg_query($con,$C1);
	$NF=pg_num_rows($R1);
	$F=pg_fetch_assoc($R1);
	if($NF>0){
	     echo "El catedratico con la Matricula: ".$txtMatricula." Ya esta Registrado <br>";
		 echo "Es el catedratico: ".$F['nombre']." ".$F['apepat']." ".$F['apemat']."<br>";
		 ?>
		<META HTTP-EQUIV="REFRESH" CONTENT="3;URL=MenuAdministrador.php">
	<?php
	}else{
		$C2="INSERT INTO catedratico
